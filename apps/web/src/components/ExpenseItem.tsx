...
function ExpenseItem({expenses, title}: {expenses: Expense[], title: string}) {

  return (
    <>
              <div className={styles.expensesOverviewHeader}>
            <p className={styles.expenseOverViewTitle}>{title}</p>
            <button><img src={menuIcon} alt="menuIcon" /></button>
            </div>
            <ul>
                    {expenses.map(el=> (
                          <li className={styles.expenseItem} key={el.id}>
                          <div className={styles.expenseItemLeft}>
                              <div className={styles.expenseItemImage} style={{backgroundColor: el.iconBackgroundColor}}>
                                <img src={el.icon} alt="cart icon" />
                              </div>
                              <div className={styles.expenseItemDetails}>
                                  <p className={styles.expenseItemTitle}>{el.expense}</p>
                                  <p className={styles.expenseItemTime}>{el.time} • {el.location}</p>
                              </div>
